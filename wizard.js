...
class PixelWizard {
    constructor() {
        ...
        this.manualPosition = null; // {top: %, left: %} when user drags
...
IF RESULT has sprite sheet/multiple characters/palette chart = WRONG`);
    ...
    handlePositionChange(btn) {
        ...
        // Clear manual position when user selects a preset
    ...
    initDragAndDrop() {
        ...
        overlay.addEventListener('mousedown', (e) => this.handleDragStart(e));
        document.addEventListener('mousemove', (e) => this.handleDragMove(e));
        document.addEventListener('mouseup', () => this.handleDragEnd());
        
        // Touch events for mobile
        overlay.addEventListener('touchstart', (e) => this.handleDragStart(e.touches[0]));
    ...
    handleDragStart(e) {
        ...
        this.overlayStartX = ((rect.left - container.left) / container.width) * 100;
        this.overlayStartY = ((rect.top - container.top) / container.height) * 100;
        
        console.log('🎯 Drag started:', { x: this.overlayStartX, y: this.overlayStartY });
    }
    ...
    handleDragMove(e) {
        ...
        const newLeft = this.overlayStartX + deltaXPercent;
        const newTop = this.overlayStartY + deltaYPercent;
        ...
        this.manualPosition = { left: newLeft, top: newTop };
    }
    ...
    handleDragEnd() {
        ...
        if (this.manualPosition) {
            console.log('✅ Drag ended - Manual position:', this.manualPosition);
            
            // Update position label
            const posLabel = document.getElementById('logoPosition');
            if (posLabel) {
                posLabel.textContent = `Manuel (${Math.round(this.manualPosition.top)}%, ${Math.round(this.manualPosition.left)}%)`;
            }
            
            // Deselect preset position buttons
            document.querySelectorAll('.position-btn').forEach(b => b.classList.remove('active'));
        }
    }
    ...
    updateMockupPosition() {
        ...
        // If user has manually positioned, use that
        if (this.manualPosition) {
            overlay.style.left = `${this.manualPosition.left}%`;
            overlay.style.top = `${this.manualPosition.top}%`;
        ...
        // Otherwise use preset position classes
        // Remove all position classes
        overlay.className = 'mockup-overlay draggable';
        ...
        if (posLabel) {
            ...
            posLabel.textContent = positionNames[this.selectedPosition] || this.selectedPosition;
        }
    ...
    getMockupUrl(view = 'front') {
        // REAL PHOTO MOCKUPS for all t-shirt colors
        ...
        if (product === 'tshirt') {
            ...
            // Fallback to center-chest if position not found
            return mockupMap['center-chest'];
        }
        
        // Fallback to SVG for other products
        // Color hex values
        const colors = {
            black: '#1a1a1a',
            white: '#f5f5f5',
